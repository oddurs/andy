...
Acceptance criteria
- [ ] XDG_CACHE_HOME, XDG_CONFIG_HOME, XDG_DATA_HOME and XDG_STATE_HOME are all honoured
- [ ] Unset behaviour is unchanged: ~/.cache, ~/.config, ~/.local/share, ~/.local/state
- [ ] A relative or empty value falls back to the default rather than producing a relative path
- [ ] expand= globs resolve through the same translation
- [ ] Tested by setting the variables and finding the files
